        ...
        <div class="live-area">
            ...
            >
                ...
                >
                    <div class="chip-area">
                        <p
                            v-if="item.stream_type === 'audio'"
                            class="gradient-background"
                        >
                            오디오
                        </p>
                        <p v-else class="gradient-background">비디오</p>
                    </div>
                </div>
                ...
                <div class="live-status-area">
                    <div
                        ...
                        :style="`background-image: url('${item.avatar_main[0]}');`"
                    ></div>
                    <div class="info-area">
    ...
    try {
        ...
        const updatedSecretDatas = await Promise.all(secretDatas.map(async secret => {
            ...
            if (userError) {
                ...
                return { ...secret, username: null, avatar_main: [] }; // 오류 발생 시 username을 null로 설정
            }

            return {
                ...secret,
                username: userData.username, // username 추가
                avatar_main: userData.avatar_main,
